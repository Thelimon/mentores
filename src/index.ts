  ...
  while (menu.isActive()) {
    ...
    switch (key) {
      ...
      case 2: // INGRESAR ESTUDIANTE
        ...
        const documentoEstudiante = await menu.getString(
          "Ingrese su numero de documento"
        );
        const telefonoEstudiante = await menu.getString(
          "Ingrese su número de teléfono"
        );
        const correoEstudiante = await menu.getString(
      ...
      case 3: // INGRESAR CONFERENCIA
        ...
        const validarMentorContraseña = await menu.getString(
          "Ingrese la contraseña"
        );

        let validarMentor: Mentor | undefined = mentorValidacion(
          listaMentores,
          validarMentorCorreo,
          validarMentorContraseña
        );

        if (validarMentor === undefined) {
          console.log("Los datos ingresados no son válidos");
        } else {
          const nombreConferencia = await menu.getString(
            "Ingrese nombre de la Conferencia"
          );
          const cantidadMaxima = 20;
          const fechaInicio = await menu.getString(
            "Ingrese la fecha de Inicio de la conferencia dd-mm-aaaa"
          );
          const fechaFinal = await menu.getString(
            "ingrese la fecha de Finalización de la conferencia dd-mm-aaaa"
          );

          let validacionEmail: Mentor[] = listaMentores.filter(
            (e) => e.getEmail() === validarMentorCorreo
          );
          ...
          if (validacionEmail.length != 0) {
            ...
            for (let i = 0; i < conferenciasMentor.length; i++) {
              let fechaInicio1: string =
                conferenciasMentor[i].fechaDeInicio.split("-")[0];
              let fechaFin1: string =
                conferenciasMentor[i].fechaDeFinalizacion.split("-")[0];
      ...
      case 4: // REGISTRARSE ESTUDIANTE A UNA CONFERENCIA
        ...
        if (validarDatos === undefined) {
          ...
          const ingresarConferencia: string = await menu.getString(
            "Ingrese el nombre de la conferencia"
          );

          const validarConferencia: Conferencia | undefined =
            listaConferencia.find(
          ...
          if (validarConferencia === undefined) {
            ...
            if (validarConferencia.getEstudiantes().length === 20) {
              ...
              const confirmarConferencia: Conferencia | undefined =
                listaConferencia.find(
                  (e) => e.nombreConferencia === ingresarConferencia
                );
              if (confirmarConferencia === undefined) {
                ...
                const ValidacionCorreoUnico = confirmarConferencia
                  .getEstudiantes()
                  .some((e) => e.getEmail() === registrarCorreo);
                if (ValidacionCorreoUnico) {
                  ...
                  console.log(
                    "registro exitoso" +
                      ...
                      registrarCorreo +
                      " en " +
                      confirmarConferencia.nombreConferencia
                  );
                }
              }
            }
      ...
      case 9: // LISTA DE ESTUDIANTES POR CONFERENCIA
        ...
        const verificarConferencia: Conferencia | undefined =
          listaConferencia.find(
            (e) => e.nombreConferencia === ingresarNombreConferencia
          );
